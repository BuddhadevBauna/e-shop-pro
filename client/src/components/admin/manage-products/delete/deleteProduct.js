...
export const handleDeleteProduct = async (productId, token, data, dispatch, categories) => {
    try {
        const deleteProductURL = import.meta.env.VITE_DELETE_PRODUCT+'/'+productId;
        const response = await axios.delete(deleteProductURL, {headers: {Authorization: `Bearer ${token}`}, data: data});
